...
namespace FibonacciRange
{
    public static class Validator
    {
        ...
        public static (int, int) Validate(string[] args)
        {
            ...
            if (args.Length == 2 && int.TryParse(args[0], out int beginNumber) && int.TryParse(args[1], out int endNumber))
            {
                if (beginNumber >= 0 && endNumber > 0 && beginNumber < endNumber)
                        return (beginNumber, endNumber);
                    else
                        throw new ArgumentException("Begin must be greater than 0 and end must be greater than 1!\nAnd begin must be less than end!");
            }
            else
